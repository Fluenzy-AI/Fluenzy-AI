...
function ConversationItem({ conversation, isSelected, onClick }: ConversationItemProps) {
  ...
  return (
    <div
      className={`group relative px-2 ${isSelected ? 'bg-purple-600/10' : ''}`}
    >
      <button
        onClick={onClick}
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          <div className="flex items-center justify-between mt-0.5">
            ...
            {conversation.unreadCount > 0 && (
              <span className="flex-shrink-0 w-5 h-5 bg-purple-600 rounded-full flex items-center justify-center text-[10px] font-bold text-white">
      ...
      <button
        ...
        className="absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-white/10 text-slate-400 transition-opacity"
      >
      ...
      <AnimatePresence>
        {showMenu && (
          <motion.div
            ...
            className="absolute right-4 top-full mt-1 w-36 py-1 bg-slate-800 rounded-lg border border-white/10 shadow-xl z-10"
          >
