      ...
      <AnimatePresence>
        {showScrollButton && (
          <motion.button
            ...
            className="fixed bottom-28 right-8 p-3 bg-purple-600 hover:bg-purple-500 text-white rounded-full shadow-lg transition-colors"
          >
  ...
  if (message.isDeleted && message.deletedForAll) {
    return (
      <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
        <div className="px-4 py-2 bg-slate-800/30 rounded-xl text-slate-500 text-sm italic">
    ...
    >
      ...
      <div className={`max-w-[70%] relative ${isOwn ? 'items-end' : 'items-start'}`}>
        ...
        {!isOwn && showAvatar && message.sender && (
          <p className="text-xs text-purple-400 mb-1 ml-1">
            {message.sender.name}
          </p>
        )}
        ...
        {/* Reply preview */}
        {message.replyTo && (
          <div className={`mb-1 px-3 py-1 rounded-lg border-l-2 ${
            isOwn ? 'border-purple-400 bg-purple-900/20' : 'border-slate-400 bg-slate-800/30'
          }`}>
            <p className="text-xs text-slate-400">{message.replyTo.senderName}</p>
            <p className="text-xs text-slate-300 truncate">{message.replyTo.content}</p>
          </div>
        )}
          ...
          {uniqueReactions.length > 0 && (
            <div className={`absolute -bottom-3 ${isOwn ? 'right-2' : 'left-2'} flex gap-0.5`}>
              {uniqueReactions.slice(0, 3).map((emoji) => (
                <span 
                  key={emoji}
                  className="w-5 h-5 bg-slate-700 rounded-full flex items-center justify-center text-xs border border-slate-800"
                >
                  {emoji}
                </span>
              ))}
              {uniqueReactions.length > 3 && (
                <span className="w-5 h-5 bg-slate-700 rounded-full flex items-center justify-center text-[10px] text-slate-300 border border-slate-800">
                  +{uniqueReactions.length - 3}
                </span>
              )}
            </div>
        ...
        <div 
          className={`absolute top-0 ${isOwn ? '-left-20' : '-right-20'} opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1`}
        >
          <button 
        ...
        <AnimatePresence>
          {showReactions && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.9 }}
              className={`absolute top-0 ${isOwn ? '-left-40' : '-right-40'} p-2 bg-slate-800 rounded-lg border border-white/10 flex gap-1 z-10`}
            >
              {['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) => (
                <button
                  key={emoji}
        ...
        <AnimatePresence>
          {showMenu && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.9 }}
              className={`absolute top-8 ${isOwn ? 'right-0' : 'left-0'} w-36 py-1 bg-slate-800 rounded-lg border border-white/10 shadow-xl z-10`}
            >
              <button className="w-full px-3 py-2 text-left text-sm text-white hover:bg-white/5 flex items-center gap-2">
                <Forward size={14} />
                Forward
              </button>
              {isOwn && (
                <>
                  ...
                  <button className="w-full px-3 py-2 text-left text-sm text-red-400 hover:bg-white/5 flex items-center gap-2">
                    <Trash2 size={14} />
                    Delete
                  </button>
                </>
...
function MessageContent({ message, isOwn }: { message: MessageWithDetails; isOwn: boolean }) {
  ...
  switch (message.type) {
    ...
    case 'VIDEO':
      return (
        <div className="max-w-xs rounded-lg overflow-hidden relative group">
          <video 
            src={message.mediaUrl || ''} 
      ...
      return (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate">{meta?.fileName || 'Document'}</p>
            <p className="text-xs opacity-60">{meta?.size || 'Download'}</p>
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-0.5 h-6">
              {(voiceMeta?.waveform || Array(30).fill(0.5)).map((h: number, i: number) => (
                <div 
                  key={i}
                  className={`w-1 bg-current rounded-full ${isPlaying ? 'animate-pulse' : ''}`}
                  style={{ 
