...
const getCompanyLogoUrl = (name: string): string => {
  const normalized = (name || '').toLowerCase().trim();
  if (normalized.includes('flipkart')) return '/companeyicon/Flipkart-Logo.wine.svg';
  if (normalized.includes('google')) return '/companeyicon/Google-Logo.wine.svg';
  if (normalized.includes('microsoft')) return '/companeyicon/Microsoft-Logo.wine.svg';
  if (normalized.includes('amazon')) return '/companeyicon/Amazon_(company)-Logo.wine.svg';
  if (normalized.includes('meta') || normalized.includes('facebook')) return '/companeyicon/Meta_Platforms-Logo.wine.svg';
  if (normalized.includes('apple')) return '/companeyicon/Apple_Inc.-Logo.wine.svg';
  if (normalized.includes('tcs') || normalized.includes('tata')) return '/companeyicon/Tata_Consultancy_Services_Logo_2020.svg';
  if (normalized.includes('infosys')) return '/companeyicon/Infosys_Consulting-Logo.wine.svg';
  if (normalized.includes('deloitte')) return '/companeyicon/deloitte.svg';
  if (normalized.includes('goldman')) return '/companeyicon/goldmansachs-ar21.svg';
  if (normalized.includes('chase') || normalized.includes('jpmorgan')) return '/companeyicon/icons8-chase-bank-480.svg';
  if (normalized.includes('capgemini')) return '/companeyicon/Capgemini-Logo.wine.svg';
  if (normalized.includes('nvidia')) return '/companeyicon/Nvidia-Logo.wine.svg';
  
  return '/companeyicon/Google-Logo.wine.svg';
};
...
export default function MobileSessionPage() {
  ...
  const rawCompanyLogo = searchParams.get('companyLogo');
  const companyLogoUrl = rawCompanyLogo ? decodeURIComponent(rawCompanyLogo) : getCompanyLogoUrl(company);
        ...
        <AnimatePresence>
          {isInterviewActive && (
            ...
            >
              ...
              >
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center shadow-sm shrink-0 p-1 border border-slate-200 overflow-hidden">
                    <img
                      src={companyLogoUrl}
                      alt={company}
                      className="max-w-full max-h-full object-contain"
                      onError={(e) => {
                        (e.currentTarget as HTMLImageElement).src = '/companeyicon/Google-Logo.wine.svg';
                      }}
                    />
                  </div>
                  <div>
