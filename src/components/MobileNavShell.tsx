...
const ACCENT: Record<string, string> = { light: "#5A2D82", parchment: "#7C3AED", dark: "#7C3AED", midnight: "#7C3AED", forest: "#F59E0B", codeterm: "#CC4125" };
const CARD_BG: Record<string, string> = { light: "#FFFFFF", parchment: "#FFFDF9", dark: "#161B2E", midnight: "rgba(15,39,68,0.9)", forest: "rgba(17,28,20,0.9)", codeterm: "#141414" };
const PAGE_BG: Record<string, string> = { light: "#F8FAFC", parchment: "#F5EFEB", dark: "#0D0F1A", midnight: "#0a1929", forest: "#0b140e", codeterm: "#0D0D0D" };
...
const BORDER_HEX: Record<string, string> = { light: "#E5E7EB", parchment: "#E2D9CF", dark: "rgba(255,255,255,0.08)", midnight: "rgba(255,255,255,0.08)", forest: "rgba(180,120,30,0.2)", codeterm: "rgba(204,65,37,0.25)" };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col sm:hidden" style={{ background: pageBgHex }}>
      ...
      <header className="flex items-center justify-between px-4 shrink-0"
        style={{ height: "56px", background: isLight ? pageBgHex : cardBgHex, borderBottom: `1px solid ${borderHex}`, boxShadow: isLight ? "0 1px 3px rgba(0,0,0,0.04)" : "0 1px 6px rgba(0,0,0,0.12)" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-center w-9 h-9 rounded-xl active:opacity-70"
            style={{ background: isLight ? "rgba(0,0,0,0.04)" : "rgba(255,255,255,0.06)", border: `1px solid ${borderHex}` }}>
            <NotificationBell isDark={!isLight} iconSize={18} iconColor={textHex} />
          </div>
