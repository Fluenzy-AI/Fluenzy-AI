...
// ═══════════════════════════════════════════════════════════════════════════════
// Super Admin - Competition Detail Page
// Full competition management with analytics
// ═══════════════════════════════════════════════════════════════════════════════
...
const scopeConfig = {
  ...
  COLLEGE: { icon: Building2, label: 'College', color: 'text-emerald-400' }
};
...
export default function SuperAdminCompetitionDetailPage({
  params
}: {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const lbRes = await fetch(`/api/competitions/${competitionId}/leaderboard?limit=50`);
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will permanently delete the competition and all associated data including participants, scores, and leaderboards. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {leaderboard.slice(0, 5).map((entry) => (
                        <div key={entry.userId} className="flex items-center justify-between p-2 bg-slate-700/50 rounded-lg">
                          <div className="flex items-center gap-3">
                            <RankBadge rank={entry.rank} size="sm" />
                            <span className="font-medium">{entry.userName}</span>
                          </div>
                          <span className="text-violet-400 font-semibold">{entry.totalScore.toFixed(1)}</span>
                        </div>
                      ))}
                    </div>
                  </CardContent>
            ...
            <div className="space-y-6">
              ...
              <Card className="bg-slate-800/50">
                ...
                <CardContent className="space-y-2">
                  <Link href={`/superadmin/competitions/${competition.id}/participants`}>
              ...
              <Card className="bg-slate-800/50">
                <CardHeader>
                  <CardTitle className="text-sm">Competition Details</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 text-sm">
                  <div>
                    <p className="text-muted-foreground">Created by</p>
                    <p>{competition.createdByName || 'System'}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Prize Pool</p>
                    <p className="text-yellow-400">{competition.prizePool || 'Certificates'}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Max Attempts</p>
                    <p>{competition.maxAttempts}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <CardContent>
              <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-slate-700/50 rounded-lg p-4 text-center">
                  ...
                  <p className="text-3xl font-bold">{competition.participantCount || 0}</p>
                ...
                <div className="bg-slate-700/50 rounded-lg p-4 text-center">
                  ...
                  <p className="text-3xl font-bold">
                    {leaderboard.length > 0 
                      ? (leaderboard.reduce((sum, e) => sum + e.totalScore, 0) / leaderboard.length).toFixed(1)
                      : '0'
                    }
                  </p>
                ...
                <div className="bg-slate-700/50 rounded-lg p-4 text-center">
                  ...
                  <p className="text-3xl font-bold">
                    {leaderboard.length > 0 ? leaderboard[0]?.totalScore.toFixed(1) : '0'}
                  </p>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-mono text-xs">{competition.id}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{competition.type.replace(/_/g, ' ')}</p>
                </div>
              </div>
