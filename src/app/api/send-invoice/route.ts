...
// ─── Helpers (mirrors /api/payment-history/[id]/pdf) ─────────────────────────
...
// ─── Build invoice HTML (mirrors /api/payment-history/[id]/pdf) ───────────────
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:28px 24px;background:#fafafa;border-radius:10px;border:1px solid #e2e8f0;">
          ...
          <p style="color:#334155;font-size:15px;margin-bottom:8px;">Hi <strong>${user.name || "there"}</strong>,</p>
          ...
          <div style="background:#f1f5f9;border-radius:8px;padding:14px 18px;margin-bottom:20px;font-size:13px;color:#334155;">
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Plan</span><strong>${payment.plan || "Standard"}</strong></div>
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span>Status</span><strong style="color:${status === "paid" || status === "free_via_coupon" ? "#16a34a" : status === "failed" ? "#dc2626" : "#d97706"}">${titleCase(status)}</strong></div>
