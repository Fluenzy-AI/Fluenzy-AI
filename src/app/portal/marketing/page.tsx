...
import {
  ...
  Plus, ChevronRight, Loader2, CheckCircle,
  Play, Pause, Eye, RefreshCw
} from "lucide-react";
...
export default function MarketingDashboardPage() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      console.log("Dashboard API response:", res.status, data);
      if (res.ok && data) {
        setDashboard(data);
      } else {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const data = await res.json();
      console.log("Campaigns API response:", res.status, data);
      if (res.ok && data) {
        setCampaigns(data.campaigns || []);
      } else {
        console.error("Campaigns API error:", data);
      }
    } catch (error) {
  ...
  const fetchSegments = async () => {
    try {
      ...
      const data = await res.json();
      console.log("Segments API response:", res.status, data);
      if (res.ok && data) {
        setSegments(data.segments || []);
      } else {
        console.error("Segments API error:", data);
      }
    } catch (error) {
  ...
  const fetchTriggers = async () => {
    try {
      ...
      const data = await res.json();
      console.log("Triggers API response:", res.status, data);
      if (res.ok && data) {
        setTriggers(data.triggers || []);
      } else {
        console.error("Triggers API error:", data);
      }
    } catch (error) {
